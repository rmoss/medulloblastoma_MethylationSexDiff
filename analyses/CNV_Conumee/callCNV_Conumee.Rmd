...
# pd_filt$MSA_Well <- NA
# pd_filt$Pool_ID <- NA
# pd_filt <- pd_filt[,c("Sample_Name","Sample_Well","MSA_Well","Sample_Plate","Sample_Group","Pool_ID","Sentrix_ID","Sentrix_Position","Basename","Title","Source_Name","supplementary_file","supplementary_file.1","Age","Bead_Chip","Brain_Region","Sex","Tissue","Basename","GSM","filename")]
# write_csv(pd_filt, file=paste0(procData,"/",controlGSE,"_pheno.csv"))
# pd_normCBL <- pd_filt %>%
#   filter(Sample_Group=="ND" & Brain_Region=="CBL")
# write_csv(pd_normCBL, file=paste0(procData,"/Control_CBL_",controlGSE,"_pheno_Sample_Sheet.csv"))
...
```{r}
datalist <- list()
s <- head(names(Newcastle.data))
for (samp in s){
  x <- CNV.fit(Newcastle.data[samp], controls.data, anno)
  x <- CNV.bin(x)
  x <- CNV.detail(x)
  x <- CNV.segment(x)
  pdf(paste0(results_dir,"/","Newcastle_test_",samp,"_CNV.pdf"),
...
write_csv(big_data, file=paste0(results_dir,"/Newcastle_CNVsegments.csv"))
pdf(file=paste0(results_dir,"/allCNVgenomeplot.pdf"))
do.call(grid.arrange, c(plist, ncol=5))
dev.off()
plist <- list()

for (samp in rownames(Newcastle.data)){
  x <- CNV.fit(Newcastle.data["GSM2459851_9761749073_R04C01"], controls.data, anno)
  x <- CNV.bin(x)
  x <- CNV.detail(x)
  x <- CNV.segment(x)
