...
# designMatrix1 <- model.matrix(~ pheno_sex + pheno_age + p_subgroup)
# dmrs_all_age <- findDMRs(GRSet_filt, designMatrix1, cut, num, opt$outDir, paste0(opt$prefix,"_age_subgroup_adj"), pheno_filt)

# Do the same but with age as a category
designMatrix2 <- model.matrix(~ pheno_sex + pheno_agecat + p_subgroup)
dmrs_all_agecat <- findDMRs(GRSet_filt, designMatrix2, cut, num, opt$outDir, paste0(opt$prefix,"_agecat_subgroup_adj"), pheno_filt)
