...
# read in the genomic ratio set & pheno data
...
print(opt$prefix)
print(grepl("Cavalli",opt$prefix))

# customized factoring for Cavalli data
if (grepl("Cavalli", opt$prefix)) {
  print("running Cavalli block")
...
# altered for SHH subtype analysis
subgroups <- c("SHH_alpha","SHH_beta","SHH_delta","SHH_gamma")
...
for (sub in subgroups){
  ...
  # read in DMP results from subtype_DMPbySex.R
  dmpSig <- read_csv(paste0(opt$dir,"/subtype_DMP/",prefix,"_ageacatadj_dmp_MethFullResults.csv"))
  dmpSig <- column_to_rownames(dmpSig, var="probeID")
  # filter for just subgroup or subtype defined above
  #pheno_sub <- filter(pheno_filt, subgroup==sub)
  pheno_sub <- filter(pheno_filt, subtype==sub)
  ...
  # create smmary of average beta values by probeID and sex
  summary2 <- betaLong2 %>% 
